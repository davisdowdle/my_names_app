...
selected_name = st.text_input('Enter a name', 'John') # default
...
girl_names = year_df[year_df['sex'] == 'F'].sort_values('n', ascending = False).head(5)['name'].reset_index(drop = True)
boy_names = year_df[year_df['sex'] == 'M'].sort_values('n', ascending = False).head(5)['name'].reset_index(drop = True)
